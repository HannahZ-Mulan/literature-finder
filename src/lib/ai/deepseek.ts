 ...
 *
 * Pricing (as of 2024): Very competitive
 * - Input: ~1 RMB/1M tokens
 * - Output: ~2 RMB/1M tokens
 */
...
Respond in JSON format with these exact keys: research_question, methodology, key_findings, limitations, future_work`;
    ...
    try {
      ...
      try {
        ...
        console.warn('[DeepSeek] Failed to parse insights JSON');
      }
      ...
      console.error('[DeepSeek] Extract insights error:', error);
      throw error;
    }
  }
...
**重要：引用标注要求**
当回答涉及论文具体内容时，请在相关句子后标注引用位置：
- 如果内容在论文开头部分，使用 [引用: 开头部分]
- 如果内容在论文中间部分，使用 [引用: 中间部分]
- 如果内容在论文结尾部分（讨论/结论），使用 [引用: 结尾部分]
- 如果是概括性内容，不需要标注引用

示例格式：
"研究发现X对Y有显著影响 [引用: 结尾部分]。这说明..."
